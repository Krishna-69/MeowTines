...
export default function Ask(props) {
  ...
  const [askText, setAskText] = useState("Itna Bussy Kyu Rheti Ho?? bussy rheti ho bhi ya nahi???");
  ...
  const handleNoClick = (event) => {
    ...
    if (noCount === 0) {
      ...
      setAskText("Nahi Jawab Dena Hai kya?????")
      setEmojiAsk('😢')
    } else if (noCount === 1) {
      ...
      setAskText("MEOW MEOW MEOW MEOW????")
      setEmojiAsk('😭')
    }
    else {
  ...
  return (
    <div className='message-sending-body flex items-center flex-col w-full h-full bg-transparent bg-gradient-to-tr from-[#F56217] to-[#0B486B] min-h-screen gap-6 sm:gap-20'>
      ...
      <div className="ask-convey px-4 flex flex-col flex-wrap items-center gap-3 sm:gap-7">
        <div className='convey h-fit w-fit text-transparent bg-clip-text  bg-gradient-to-t from-[#962820] to-[#111125] sm:text-4xl text-center select-none'>In a world brimming with over 3.95 billion extraordinary Meows, you stand out to Meow as exceptionally beautiful and charming meow.....kuch zada chezzy ho gaya hai kya??".</div>
